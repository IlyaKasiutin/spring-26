...
Вектор счётов по классам: к индексу класса $c_L$ добавляют $q_L$, к $c_R$ — $q_R$; итоговый класс — $\operatorname{argmax}$ по этому вектору. Так трактуется смешивание предсказаний дочерних ветвей по условным вероятностям направления при неизвестном значении признака в узле.
